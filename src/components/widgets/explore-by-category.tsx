...
export function ExploreByCategory({ categories }: Props) {
  ...
  return (
    <Card className="p-4">
      <h2 className="mb-3 text-sm font-bold">{t("widgets.exploreByCategory", "Explore by Category")}</h2>
      <div className="grid grid-cols-3 gap-2">
        {shown.map((category) => (
          ...
          >
            ...
            <span className="text-[11px] font-medium leading-tight">{translateCategory(category.name)}</span>
          </Link>
        ))}
        <Link
          href="/explore"
